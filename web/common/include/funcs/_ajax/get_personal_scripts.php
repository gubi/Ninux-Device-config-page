	...
	if($check_user_id->execute()){
		if($check_user_id->rowCount() > 0){
			while($dato_user = $check_user_id->fetch()){
				...
				$check_session = $pdo->prepare("select * from `ninux_cp_personal_configs` where `user_id` = '" . addslashes($user_id) . "' order by `date` desc");
				if($check_session->execute()){
					if($check_session->rowCount() > 0){
						...
						while($dato_session = $check_session->fetch()){
							...
							$config_items["script"][$id]["title"] = stripslashes(stripslashes($dato_session["title"]));
							$config_items["script"][$id]["config"] = stripslashes(stripslashes($dato_session["config"]));
