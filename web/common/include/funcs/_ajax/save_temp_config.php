			...
			while($dato_user = $check_user_id->fetch()){
				...
				if($check_session->execute()){
					if($check_session->rowCount() > 0){
							...
							if($check_changes->rowCount() > 0){
								while($dato_changes = $check_changes->fetch()){
									if(trim($_POST["script"]) !== $dato_changes["config"] && trim($_POST["title"]) !== $dato_changes["title"]){
										$update = $pdo->prepare("update `ninux_cp_personal_configs` set `title` = '" . addslashes($title) . "', `config` = '" . addslashes(trim($_POST["script"])) . "', `is_temp` = '1' where `user_id` = '" . addslashes($user_id) . "' and `session_id` = '" . $session_id . "'");
										if($update->execute()){
											print "ok";
										} else {
						...
						if($save->execute(array(":user_id" => addslashes($user_id), ":session_id" => addslashes($_POST["session_id"]), ":title" => addslashes($title), ":config" => addslashes(trim($_POST["script"])), ":is_temp" => "1"))){
							print "ok";
						} else {
